...
exports.requirePackage = exports.requireEditions = exports.requireEdition = exports.loadEdition = void 0
// Imports
 ...
 */
var BLACKLIST =
	((_a = process.env.EDITIONS_TAG_BLACKLIST) === null || _a === void 0
		? void 0
		: _a.split(/[, ]+/g)) ||
	((_b = process.env.EDITIONS_SYNTAX_BLACKLIST) === null || _b === void 0
		? void 0
		: _b.split(/[, ]+/g))
/**
...
if (BLACKLIST) {
	for (var i = 0; i < BLACKLIST.length; ++i) {
		...
		blacklist[tag] = util_js_1.errtion({
			message:
				'The EDITIONS_TAG_BLACKLIST (aka EDITIONS_SYNTAX_BLACKLIST) environment variable blacklisted the tag [' +
				tag +
				']',
...
function requireEdition(edition, opts) {
	...
	if (
		...
		throw util_js_1.errtion({
			message:
				'Each edition must have its [description, directory, entry, engines] fields defined, yet all it had was [' +
	...
	if (opts.strict == null) {
		try {
			return requireEdition(
				edition,
				__assign(__assign({}, opts), { strict: true })
			)
		} catch (err) {
			return requireEdition(
				edition,
				__assign(__assign({}, opts), { strict: false })
			)
		}
	}
	// Verify tag support
			...
			throw util_js_1.errtion(
				{
					message:
						'Skipping edition [' +
						edition.description +
						'] because it contained a blacklisted tag [' +
						tag +
						']',
	...
	if (opts.strict) {
		if (edition.engines.node === true) {
			...
			semver_1.default.satisfies(NODE_VERSION, edition.engines.node) === false
		) {
			throw util_js_1.errtion({
				message:
					'Skipping edition [' +
					edition.description +
					'] because our current node version [' +
					NODE_VERSION +
					'] is not supported by its specific range [' +
					util_js_1.stringify(edition.engines.node) +
					']',
		...
		if (semver_1.default.satisfies(NODE_VERSION, simplifiedRange) === false) {
			throw util_js_1.errtion({
				message:
					...
					'] because our current node version [' +
					NODE_VERSION +
					'] is not supported by its simplified range [' +
					util_js_1.stringify(simplifiedRange) +
					']',
...
function requireEditions(editions, opts) {
	...
	if (!editions || editions.length === 0) {
		if (opts.packagePath) {
			throw util_js_1.errtion({
				message:
					'There were no editions specified for package [' +
					opts.packagePath +
					']',
	...
	// otherwise, provide the error
		...
		if (opts.packagePath) {
			throw util_js_1.errtion(
				{
					message:
						'There were no suitable editions for package [' +
						opts.packagePath +
						']',
