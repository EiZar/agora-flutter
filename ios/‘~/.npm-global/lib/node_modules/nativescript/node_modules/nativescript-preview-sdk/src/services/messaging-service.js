...
var MessagingService = /** @class */ (function () {
    ...
    MessagingService.prototype.getPublishPromise = function (channel, messageType, chunks, deviceIdMeta, platform, hmrMode) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (chunks.length === 1) {
                ...
                var data = chunks.map(function (chunk) { return chunk.data; }).join("");
                var requestBody = void 0;
                if (_this.config.largeFilesProtocol === MessagingService.LARGE_SESSIONS_PROTOCOL) {
                    var decoded = _this.helpersService.base64Decode(data);
                    var parts = JSON.parse(decoded);
                    var options_1 = { codec: msgpack.createCodec({ binarraybuffer: true }) };
                    var header = parts.map(function (part) {
                        var result = {
                            file: part.file,
                            event: part.event,
                            binary: part.binary,
                            hash: part.binary === true
                                ? _this.helpersService.calculateMD5(_this.helpersService.stringToByteArray(part.fileContents, true))
                                : _this.helpersService.calculateMD5(part.fileContents)
                        };
                        var hmrHash = _this.getHMRHash(part);
                        if (hmrHash) {
                            result["hmrHash"] = hmrHash;
                        }
                        return result;
                    });
                    var encodedHeader = msgpack.encode(header, options_1);
                    var payloads = parts
                        .filter(function (part) { return part.event == "change" || part.event == "add"; })
                        .map(function (part) { return _this.helpersService.stringToByteArray(part.fileContents, part.binary).buffer; });
                    var encodedPayloads = payloads.map(function (payload) { return msgpack.encode(payload, options_1); });
                    requestBody = (_a = _this.helpersService).concatenateArrays.apply(_a, [encodedHeader].concat(encodedPayloads));
                }
                else {
                    requestBody = _this.helpersService.stringToByteArray(data);
                }
                _this.config.callbacks.onBiggerFilesUpload(requestBody, function (uploadedFilesLocation, error) {
                    if (error) {
                        reject(error);
                    }
                    else {
                        ...
                        if (typeof _this.config.largeFilesProtocol !== "undefined") {
                            message["protocol"] = _this.config.largeFilesProtocol;
                        }
                        _this.pubNub.publish({
                            message: message,
                            channel: channel,
    ...
    MessagingService.prototype.getHMRHash = function (part) {
        if (!part || !part.fileContents || !part.file || !part.file.toLowerCase().endsWith("bundle.js")) {
            return null;
        }
        var matches = MessagingService.HMR_HASH_PATTERN.exec(part.fileContents);
    ...
    MessagingService.LARGE_SESSIONS_PROTOCOL = "msg-pack";
    MessagingService.HMR_HASH_PATTERN = new RegExp("var\\s+hotCurrentHash\\s+=\\s*\\\"([^\"]*)\\\";", "g");
    MessagingService.PubNubInitialized = false;
