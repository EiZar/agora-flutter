...
class LiveSyncCommandHelper {
    ...
    createDeviceDescriptors(devices, platform, additionalOptions) {
        return __awaiter(this, void 0, void 0, function* () {
            const deviceDescriptors = devices.map((d) => {
                ...
                const buildData = this.$buildDataService.getBuildData(this.$projectData.projectDir, d.deviceInfo.platform, Object.assign(Object.assign({}, this.$options.argv), { outputPath, buildForDevice: !d.isEmulator, watch: !this.$options.release && this.$options.watch }));
    ...
    getPlatformsForOperation(platform) {
        const availablePlatforms = platform
            ? [platform]
            : _.values(this.$mobileHelper.platformNames.map((p) => p.toLowerCase()));
    ...
    runInRelease(platform, deviceDescriptors) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const deviceDescriptor of deviceDescriptors) {
                ...
                yield device.applicationManager.startApplication({
                    appId: this.$projectData.projectIdentifiers[device.deviceInfo.platform.toLowerCase()],
