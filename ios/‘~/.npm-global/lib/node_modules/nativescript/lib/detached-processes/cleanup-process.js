...
const executeRequest = (request) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        fileLogService.logData({
            message: `Finished executing request: ${request.method} ${request.url} and got status ${responseStatus}`,
        });
    }
    catch (e) {
        fileLogService.logData({
...
const addCleanupAction = (commandInfo) => {
    if (_.some(commandsInfos, (currentCommandInfo) => _.isEqual(currentCommandInfo, commandInfo))) {
        fileLogService.logData({
            message: `cleanup-process will not add command for execution as it has been added already: ${JSON.stringify(commandInfo)}`,
        });
    }
    else {
        fileLogService.logData({
            message: `cleanup-process added command for execution: ${JSON.stringify(commandInfo)}`,
        });
...
const removeCleanupAction = (commandInfo) => {
    if (_.some(commandsInfos, (currentCommandInfo) => _.isEqual(currentCommandInfo, commandInfo))) {
        ...
        fileLogService.logData({
            message: `cleanup-process removed command for execution: ${JSON.stringify(commandInfo)}`,
        });
    }
    else {
        fileLogService.logData({
            message: `cleanup-process cannot remove command for execution as it has not been added before: ${JSON.stringify(commandInfo)}`,
        });
    }
};
const addRequest = (requestInfo) => {
    if (_.some(requests, (currentRequestInfo) => _.isEqual(currentRequestInfo, requestInfo))) {
        fileLogService.logData({
            message: `cleanup-process will not add request for execution as it has been added already: ${JSON.stringify(requestInfo)}`,
        });
    }
    else {
        fileLogService.logData({
            message: `cleanup-process added request for execution: ${JSON.stringify(requestInfo)}`,
        });
...
const removeRequest = (requestInfo) => {
    if (_.some(requests, (currentRequestInfo) => _.isEqual(currentRequestInfo, currentRequestInfo))) {
        ...
        fileLogService.logData({
            message: `cleanup-process removed request for execution: ${JSON.stringify(requestInfo)}`,
        });
    }
    else {
        fileLogService.logData({
            message: `cleanup-process cannot remove request for execution as it has not been added before: ${JSON.stringify(requestInfo)}`,
        });
    }
};
const addDeleteAction = (filePath) => {
    ...
    if (_.some(filesToDelete, (f) => f === fullPath)) {
        fileLogService.logData({
            message: `cleanup-process will not add ${fullPath} for deletion as it has been added already`,
        });
    }
    else {
        filesToDelete.push(fullPath);
        fileLogService.logData({
            message: `cleanup-process added ${fullPath} for deletion`,
        });
    }
};
const removeDeleteAction = (filePath) => {
    ...
    if (_.some(filesToDelete, (f) => f === fullPath)) {
        ...
        fileLogService.logData({
            message: `cleanup-process removed ${fullPath} from the list of files for deletion.`,
        });
    }
    else {
        fileLogService.logData({
            message: `cleanup-process cannot remove ${fullPath} for deletion as no such entry is found in the files marked for deletion`,
        });
    }
};
const addJSFile = (jsCommand) => {
    ...
    if (_.some(jsCommands, (currentJSCommand) => _.isEqual(currentJSCommand, jsCommand))) {
        fileLogService.logData({
            message: `cleanup-process will not add JS file for execution as it has been added already: ${JSON.stringify(jsCommand)}`,
        });
    }
    else {
        fileLogService.logData({
            message: `cleanup-process added JS file for execution: ${JSON.stringify(jsCommand)}`,
        });
...
const removeJSFile = (jsCommand) => {
    ...
    if (_.some(jsCommands, (currentJSCommand) => _.isEqual(currentJSCommand, jsCommand))) {
        ...
        fileLogService.logData({
            message: `cleanup-process removed JS action for execution: ${JSON.stringify(jsCommand)}`,
        });
    }
    else {
        fileLogService.logData({
            message: `cleanup-process cannot remove JS action for execution as it has not been added before: ${JSON.stringify(jsCommand)}`,
        });
    }
};
